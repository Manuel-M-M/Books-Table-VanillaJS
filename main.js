...
class Table {
    ...
    createRow(cells) {
        ...
        this.removable && newRow.appendChild(this.createButtonCell());
    ...
    update () {
        ...
        for (const data of this.displayedArray)
    ...
    filter (searchText) {
        ...
        this.displayedArray = this.dataArray.filter(item => {
            if (item.title.toUpperCase().includes(searchText.toUpperCase())) {
                return true;
            } else {
                return false;
            }
        ...
        // Opción 3: De forma similar a la opción 2 pero en una única línea:
        this.displayedArray = this.dataArray.filter(item => 
